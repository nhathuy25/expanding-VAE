...
- **Configurations:**
  ...
  - **Approach 2:**  
    - The model is initialized from the target fixed structure (256 and 128 neurons in encoder's 2 layers respectively) and then suddenly reduced to 64,32 neurons at the first adjustment at the 25th epoch, following a similar growth strategy.
...
- **ELBO Estimation:**  
  - ELBO is computed by averaging over 20 samples from the latent space for each mini-batch during training (the reason behind unsmooth ELBO's graph pattern).
...
- **Evaluation Metrics:**  
  - The assessment of performance was mainly based on the evolution of the loss and ELBO.  
...
- **Implementation Specifics:**  
  - The expanding method relies on He initialization and dynamic layer adjustment, which might introduce transient training instabilities.
